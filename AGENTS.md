...
- Live tests spawn real OpenCode servers and model requests: `OPENCODE_LIVE_TESTS=1 DISPLAY=:1 test/run.sh`. Do not enable them for routine verification. `ServerPortLiveTest` is the exception that needs only `opencode` on `PATH`, no provider and no model request.
- `ServerStreamTest` runs with short `-Dopencode.eventTimeoutSeconds` / `-Dopencode.stallTimeoutSeconds` overrides so the SSE bounds are observable in seconds. Those properties exist for tests and diagnostics; production defaults live in `OpenCodeService`.
- Without a window manager (a bare Xvfb display), the SWT `AttachedFilesBarTest` can block forever after emitting `Gtk-CRITICAL … GtkScrollbar` warnings, and `Browser` tests fail outright unless WebKitGTK is installed. Neither is a code failure; run the remaining SWT tests individually in such environments.
...
- The public update site is deployed by `.github/workflows/publish-update-site.yml` to `https://philipp0205.github.io/eclipse-opencode/` after changes reach `master`. The workflow publishes the complete generated repository as one GitHub Pages artifact, so stale files are removed automatically.
- GitHub Pages must be enabled once by a repository owner with **GitHub Actions** selected under Settings → Pages → Source. The workflow's `GITHUB_TOKEN` cannot enable Pages itself.
- The Pages workflow validates synchronized source versions and the generated `content.jar`. When the deployed site already has the same semantic version, it skips deployment rather than publishing a qualifier-only update; bump the semantic version for every public update.
...
- Internal mirror sequence: bump the semantic version, run `JAVA_HOME=/usr/lib/jvm/java-21 ./mvnw -q -DskipTests package`, then publish with `rsync -a --delete com.opencode.eclipse.repository/target/repository/ /home/phkurrle/public/eclipse-opencode-update-site/`.
...
- Never start the server with `--port 0`. OpenCode stopped treating port 0 as "let the OS pick" and binds its default port 4096 instead, falling back to a random port only when 4096 is taken — which makes Eclipse fight the user's own TUI/desktop for one port. `OpenCodeService` reserves a loopback port itself, passes it explicitly, and retries with a fresh port when the bind is lost, because the CLI exits with `ServeError` rather than falling back.
- Treat the `serve` banner as unstable: match the printed loopback URL (preferring the requested port), not the word "listening". Startup then polls `/global/health` instead of assuming the server can serve as soon as it prints, and keeps the process output so failures can quote what OpenCode actually said.
- OpenCode supports opt-in HTTP basic auth through `OPENCODE_SERVER_PASSWORD`, and its startup warning nudges users into setting it. Every request (REST, SSE, prompt POST) must carry `Basic opencode:<password>` when the variable is set, or the whole plugin gets 401s. Builds that print a generated password are honored too.
- Never block on an SSE subscription without bounding the handshake. The body must stay open for a whole turn, so requests bound only the headers via `sendAsync(...).get(timeout)`; a plain `send` turns a stalled server into a chat view that stays "Thinking" until Eclipse restarts.
- A turn also ends when the stream goes silent *and* `/session/status` reports the session is no longer running. Both conditions are required so a long, quiet model call is never cut short.
